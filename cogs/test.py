...
class test(commands.Cog):
    ...
    async def report(self, interaction: discord.Interaction, pkmn_name: str) -> None:
        ...
        big_ass_string = ''.join([p.user.mention for p in global_data.servers[interaction.guild.id].players.values() if p.interested(pkmn_id)])
    ...
    async def xable(self, interaction: discord, argument: str, enable: bool) -> None:
        ...
        if interaction.user.id not in global_data.servers[interaction.guild.id].players:
            global_data.servers[interaction.guild.id].players[interaction.user.id] = player.Player(interaction.user, not enable)

        if global_data.servers[interaction.guild.id].players[interaction.user.id].set_preference(argument, enable):
            interaction.response.send_message(f"Your preference for {argument} was already set that way !")
        else:
            interaction.response.send_message(f"Changed your preference for {argument} !")
